defmodule ElixirBetWeb.LandPageLive do
  ...
  def mount(_params, _session, socket) do
    matches = MatchLive.all_matches()
    {:ok, assign(socket, matches: matches)}
  end

  def mount(%{match_id: match_id}, socket) do
...
<div class="flex">
  ...
  <div class="ml-4 mr-64">
      <div class="mb-1">
          <h6 class="text-sm font-semibold"> <%= get_home_team(match.home_team_id) %></h6>
          <h6 class="text-sm font-semibold"><%= get_away_team(match.away_team_id) %></h6>
      </div>
      <div class="mb-2">
  ...
  <!-- Buttons -->
  <div class="flex">
      <button id="home_<%= match.id %>" class="bg-black hover:bg-orange-700 border border-orange-700 text-white font-bold  px-4 rounded mr-2" onclick="addToBetSlip(<%= match.id %>, 'home')"><%= match.home_odd%></button>
      <button id="draw_<%= match.id %>" class="bg-black hover:bg-orange-700 border border-orange-700 text-white font-bold  px-4 rounded mr-2" onclick="addToBetSlip(<%= match.id %>, 'draw')"><%= match.draw_odd %></button>
      <button id="away_<%= match.id %>" class="bg-black hover:bg-orange-700 border border-orange-700 text-white font-bold  px-4 rounded" onclick="addToBetSlip(<%= match.id %>, 'away')"><%= match.away_odd%></button>
  </div>
</div>
<% end %>
</div>
