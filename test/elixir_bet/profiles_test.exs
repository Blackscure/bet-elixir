defmodule ElixirBet.ProfilesTest do
  use ElixirBet.DataCase

  alias ElixirBet.Profiles

  describe "profiles" do
    alias ElixirBet.Profiles.Profile

    import ElixirBet.ProfilesFixtures

    @invalid_attrs %{msisdn: nil}
    ...
    test "create_profile/1 with valid data creates a profile" do
      valid_attrs = %{msisdn: "some msisdn"}

      assert {:ok, %Profile{} = profile} = Profiles.create_profile(valid_attrs)
      assert profile.msisdn == "some msisdn"
    end
    ...
    test "update_profile/2 with valid data updates the profile" do
      profile = profile_fixture()
      update_attrs = %{msisdn: "some updated msisdn"}

      assert {:ok, %Profile{} = profile} = Profiles.update_profile(profile, update_attrs)
      assert profile.msisdn == "some updated msisdn"
    end
